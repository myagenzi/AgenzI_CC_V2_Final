...
export function ServiceAccordion({ items, dimmed = false }: Props) {
  ...
  useScrollSetup(wrapRef as React.RefObject<HTMLElement>, (el) => {
                ...
                <div className="col-span-12 flex flex-wrap content-start gap-2 md:col-span-5">
                  {it.tags.map((t, i) => (
                    <span
                      key={i}
                      className={cn(
                        "font-mono-tech inline-flex items-center rounded-full border px-3 py-1.5 text-[10px] uppercase tracking-[0.18em] animate-fade-in",
                        t.kind === "price" &&
